...
- 複数のアカウントを利用する際の課題  
  ...
  - そこで.ssh/configと.git/configの設定を書き換えてアクセスできるようにする。  
    なお、SourceTreeを介してPuTTYを利用する場合は、下記のような複雑な手順は不要(名前/メールアドレスはSourceTree内で設定可能)。
...
4. (番外編)SourceTreeとPuTTYを利用しつつ、名前とメールアドレスを変更する。

    1. SourceTreeを起動 -> 対象のリポジトリを選択 -> 設定 -> 詳細  
         -> "グローバルユーザー設定を使う"のチェックを外す -> "フルネーム"と"メールアドレス"を変更する。
